...
Color Raytracer::CalnReflection(CollidePrimitive collide_primitive , Vector3 ray_V , int dep , int* hash ) {
	...
	ray_V = ray_V.Reflect( collide_primitive.N );//已经反向的射线
	Primitive* primitive = collide_primitive.collide_primitive;
	...
	else
	{
		...
		//NEED TO IMPLEMENT
		ray_V = ray_V.GetUnitVector();//规范化方向
		double projectionZ = ray_V.Dot(Vector3( 0. , 0. , 1. ));//Z轴方向上的投影
		Vector3 normalVec = Vector3(0., 0., 1.) * ray_V;   
		
		std::pair<double, double> tempBlur = primitive->GetMaterial()->blur->GetXY();
		Vector3 blurVec = Vector3(tempBlur.first, tempBlur.second, 0.);
		Vector3 thev = blurVec.Rotate(normalVec, acos(projectionZ));

		return RayTracing(collide_primitive.C, ray_V + thev*0.05, dep + 1, hash) * primitive->GetMaterial()->color * primitive->GetMaterial()->refl;
		//ADD BLUR
